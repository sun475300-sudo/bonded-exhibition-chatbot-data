...
KNOWN_TERMS: set[str] = {
    ...
    # ── EXHIBITION (전시) ──
    "전시", "전시장", "전시회", "전시물", "전시용", "전시용품", "전시목적",
    "전시기간", "장치", "진열", "디스플레이", "박람회", "전람회",
    "시연", "데모", "시범", "체험", "교체", "변경", "현장",
...
def correct_query(query: str) -> tuple[str, list[dict]]:
    ...
    for token in tokens:
        ...
        # 짧은 토큰(2글자 이하)은 편집 거리 1 이내로만 교정
        # (2글자는 치환 한 번으로도 완전히 다른 단어가 되므로 보수적으로 처리)
        max_dist = 1 if len(token) <= 2 else 2
        suggestion = correct_term(token, max_distance=max_dist)
        if suggestion is not None and suggestion != token:
            ...
            # (예: "신청은"→"신청서"는 2/3=0.67로 거부되어 조사를 가진 단어를 보존)
